"""
Nuskhuri OCR — Class-based utility
----------------------------------

This module wraps the dynamic CNN classifier and the multi-threshold OCR pipeline into a
single, reusable class for Nuskhuri script recognition. You can:
    ...
    def __init__(
        ...
        for level in range(num_levels):
            ...
            for _ in range(blocks_per_level):
                ...
                layers.append(nn.ReLU())  # Note: No inplace=True like in training code
    ...
    def draw_predictions(
        ...
        for (x, y, w, h), pred in zip(boxes, predictions):
            if pred in skip_labels:
                continue
            # rectangle - use blue for Nuskhuri to distinguish from Asomtavruli (red)
            draw.rectangle([x, y, x + w, y + h], outline="blue", width=1)
            # label above box when possible
            text_y = y - 15 if y - 15 >= 0 else y + h + 2
            draw.text((x, text_y), pred, fill="darkblue", font=font)
