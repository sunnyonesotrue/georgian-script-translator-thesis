...
# ✅ CONFIGURATION
# ----------------------------
model_path = r"/Users/sunnysideup/Documents/data for ORCs/All Code/Neural Networks/Saves/model_try9_40Epochs_0.0005lr_0.25dropoutRate_128BatchSize_97.14.pth"
...
# ----------------------------
num_classes = 39  # <- Force match model checkpoint
base_dataset = datasets.ImageFolder(data_path)
actual_class_count = len(base_dataset.classes)
if actual_class_count != num_classes:
    print(f"⚠️ WARNING: Model expects {num_classes} classes, but dataset has {actual_class_count}")
...
def segment_letters_from_binary(binary_img):
    ...
    boxes = sorted(boxes, key=lambda b: (b[1] // 10, b[0]))  # vertical priority, then left to right
...
def classify_segment_top1(crop, model, transform, device, idx_to_class):
    pil_img = Image.fromarray(crop).convert("L")  # not "RGB"
    tensor = transform(pil_img).unsqueeze(0).to(device)
    with torch.no_grad():
        output = model(tensor)
        pred = torch.argmax(output, dim=1).item()
        return idx_to_class.get(pred, f"[{pred}]")

def paint_predictions_on_image(original_img, segments, predictions):
    pil_img = Image.fromarray(cv2.cvtColor(original_img, cv2.IMREAD_COLOR))
...
def run_ocr_on_image_all_thresholds(image_path, output_dir):
    ...
    thresholded_images = manager.threshold_variants_from_image(image)
